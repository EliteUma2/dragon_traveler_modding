...
namespace Resolver::Protection {
	Il2CppObject* SafeRuntimeInvoke(const MethodInfo* method, Il2CppObject* obj, void** params)
	{
		...
		if (!success) {
			DebugLog("SafeRuntimeInvoke SEH CRASH: method=%s obj=0x%p", method->name ? method->name : "???", obj);
		}
		else if (exc) {
			DebugLog("SafeRuntimeInvoke IL2CPP EXCEPTION: method=%s obj=0x%p", method->name ? method->name : "???", obj);
		}
...
namespace Resolver::Helpers{
	...
	std::string GetSceneName(Il2CppObject* gameObject) {
		...
		Resolver::Protection::safe_call([&]() {
			...
			if (getSceneName) {
				void* unboxedScene = il2cpp_object_unbox(sceneBoxed);

				Il2CppString* sName = (Il2CppString*)il2cpp_runtime_invoke(getSceneName, unboxedScene, nullptr, nullptr);
				if (sName) {
