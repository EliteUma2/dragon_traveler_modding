...
export function registerMemoryTools(server: McpServer) {
  server.tool(
    "mem_dump",
    "Hex dump of raw memory at an address",
    {
  ...
  server.tool(
    "mem_scan",
    "Scan method body for CMP instructions (proto field number discovery)",
    {
  ...
  server.tool(
    "method_info",
    "Dump the MethodInfo struct for a method (address, invoker, params, flags)",
    {
      ns: z.string().describe("Namespace"),
      name: z.string().describe("Class name"),
      method: z.string().describe("Method name"),
  ...
  server.tool(
    "vtable",
    "List all vtable entries for a class",
    {
      ns: z.string().describe("Namespace"),
      name: z.string().describe("Class name"),
    },
  ...
  server.tool(
    "find_pe",
    "Scan process memory for PE files (MZ headers)",
    {},
    async () => json(await get("/api/findpe"))
  );

  server.tool(
    "dump_pe",
    "Dump raw PE bytes at address (returns base64-encoded data)",
    { addr: z.string().describe("Address of PE header (hex)") },
    async ({ addr }) => json(await getRaw("/api/dumppe", { addr }))
  );

  server.tool(
    "rc4_debug",
    "Get RC4 encryption debug log. Shows S-box state, plaintext, ciphertext for each RC4.Encrypt call. Hook must be installed first via rc4_hook.",
    {
  ...
  server.tool(
    "rc4_hook",
    "Install or uninstall the RC4.Encrypt Detours hook for debugging. Once installed, all RC4.Encrypt calls are logged with full S-box state snapshots.",
    {
  ...
  server.tool(
    "price_hook",
    "Manage the UploadHandlerRaw hook. Actions: install, uninstall, enable, disable, set (field override), remove (field override), clear (all overrides), list.",
    {
    ...
    async ({ action, field, value }) => {
      ...
      if (field !== undefined) parts.push(`field=${encodeURIComponent(field)}`);
      if (value !== undefined) parts.push(`value=${encodeURIComponent(value)}`);
  ...
  server.tool(
    "price_hook_log",
    "Get the price hook intercept log. Shows all UploadHandlerRaw bodies seen, with original and modified content.",
    {
