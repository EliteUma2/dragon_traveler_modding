...
function trimDecoded(obj, maxDepth = 3, depth = 0) {
    ...
    if (depth >= maxDepth) {
        ...
        if (keys.length > 5)
            return `{${keys.length} fields: ${keys.slice(0, 5).join(", ")}...}`;
        return obj;
    }
    const out = {};
    const keys = Object.keys(obj);
    for (const k of keys) {
        ...
        if (k.startsWith("has") && typeof obj[k] === "boolean" && obj[k] === true)
            continue;
        if (k.startsWith("has") && typeof obj[k] === "boolean" && obj[k] === false)
            continue;
...
let msgDefsCache = null;
let msgDefsCacheTime = 0;
async function getMsgDefs() {
    const now = Date.now();
    if (msgDefsCache && now - msgDefsCacheTime < 60000)
        return msgDefsCache;
    try {
        msgDefsCache = await get("/api/msgdefs");
...
async function tryDecode(msgId, hexData) {
    try {
...
export function registerPacketTools(server) {
    server.tool("send_packet", "Send a raw packet to the game (msgId + hex-encoded proto data)", {
    ...
    server.tool("capture_packets", "Poll recently captured packets with auto-decode. Returns packets with message names and decoded proto fields. Use 'since' for incremental polling. Recv packets are decoded using the game's own deserializer; send packets include raw hex (they have a 20-byte transport header).", {
        ...
        for (const pkt of packets) {
            ...
            // Apply filters
            if (filterMsgId !== undefined && pkt.msgId !== filterMsgId)
                continue;
            if (filterDir && pkt.dir !== filterDir)
                continue;
            if (filter && !name.toLowerCase().includes(filter.toLowerCase()))
                continue;
            if (exclude) {
                const excludeParts = exclude.toLowerCase().split(",").map((s) => s.trim());
                if (excludeParts.some(ex => name.toLowerCase().includes(ex)))
                    continue;
            }
            const entry = {
                seq: pkt.seq,
        ...
        for (let i = 0; i < decodePromises.length; i += 10) {
            ...
            for (let j = 0; j < batch.length; j++) {
                ...
                if (d && d.fields) {
                    ...
                    results[batch[j].idx].data = packets.find((p) => p.seq === results[batch[j].idx].seq)?.data;
                }
            }
        }
        return json(results);
