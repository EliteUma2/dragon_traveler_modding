...
export function registerInstanceTools(server) {
    server.tool("find_instances", "Find live instances of a class (FindObjectsOfType) with addresses and field values", {
    ...
    server.tool("read_field", "Read a field value from a live object at an address", {
        addr: z.string().describe("Object address (hex, e.g. '0x1234abcd')"),
        field: z.string().describe("Field name"),
    }, async ({ addr, field }) => json(await get("/api/readfield", { addr, field })));
    server.tool("write_field", "Write a field value on a live object", {
    ...
    server.tool("write_static", "Write a static field value on a class", {
        ns: z.string().describe("Namespace"),
        name: z.string().describe("Class name"),
        field: z.string().describe("Field name"),
        value: z.string().describe("New value"),
    }, async ({ ns, name, field, value }) => json(await post("/api/writestatic", { ns, name: name, field, value })));
    server.tool("invoke_method", "Invoke a method at runtime on a class or instance", {
        ns: z.string().describe("Namespace"),
        name: z.string().describe("Class name"),
    ...
    }, async ({ ns, name, method, args, instance }) => json(await post("/api/invoke", { ns, class: name, method, args, instance })));
}
