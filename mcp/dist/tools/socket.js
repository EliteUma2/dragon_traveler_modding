...
function computeCRC32(data, start = 0, end) {
    const len = end ?? data.length;
    let crc = 0xffffffff;
...
// ─── Big-endian helpers ──────────────────────────────────────────────────────
function writeBE32(buf, value, offset) {
...
function createGameSocket(host, port) {
...
function processRecvBuffer(gs) {
    while (true) {
        if (gs.recvBuf.length < 4)
            return;
        const frameLen = readBE32(gs.recvBuf, 0);
        if (gs.recvBuf.length < 4 + frameLen)
            return;
...
function sendPacket(gs, msgId, protoHex) {
    if (!gs.connected)
        throw new Error("Socket not connected");
    if (!gs.readKey)
        throw new Error("Handshake not complete — no hello received yet");
...
// ─── MCP Tools ───────────────────────────────────────────────────────────────
export function registerSocketTools(server) {
    server.tool("socket_connect", "Open a raw TCP connection to a game server. Returns socket ID for subsequent operations. Performs RC4 key handshake automatically.", {
    ...
    server.tool("socket_send", "Send a packet through an established socket connection. Builds transport frame (CRC32 + AckId + Counter + MsgId), RC4-encrypts, and sends.", {
        ...
        if (!gs)
            return json({ error: `Socket ${socketId} not found` });
        try {
    ...
    server.tool("socket_recv", "Read received packets from a socket's queue. Packets are automatically RC4-decrypted and parsed into AckId + MsgId + proto payload.", {
        socketId: z.number().describe("Socket ID from socket_connect"),
    ...
    server.tool("socket_close", "Close a socket connection and clean up resources.", {
    ...
    server.tool("socket_status", "Get status of all open sockets or a specific socket.", {
    ...
    server.tool("socket_raw_recv", "Read raw bytes received (before framing/decryption). Useful for debugging the handshake or unknown protocols.", {
